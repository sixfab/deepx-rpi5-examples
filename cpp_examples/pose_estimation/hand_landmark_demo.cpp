// ============================================================
// hand_landmark_demo.cpp  --  YOLOv11-HandPose 21-keypoint Detection
// Non-PPU float tensor: [1, 68, N] where 68 = 5 + 21*3.
// Usage: ./hand_landmark_demo [--model path] [--source video|camera]
...
public:
    explicit HandLandmarkDetector(const DemoParams& p)
          ...
          m_conf(p.confThreshold), m_iou(p.iouThreshold)
    {
        ...
        if (!m_engine) throw std::runtime_error("Failed to load hand landmark model");
    }
    ...
    std::vector<ppu::PoseDetection> infer(const cv::Mat& bgr) {
        ...
        auto dets = ppu::decodePoseFloat(outs, m_inputW, m_inputH,
                                         m_conf, m_iou, /*kptCount=*/21);
...
int main(int argc, char** argv) {
    ...
    const std::vector<std::string> labs = {"hand"};

    HandLandmarkDetector model(p);
    ...
    runDemo(source, [&](cv::Mat& frame) {
        ...
        for (const auto& d : dets) {
            vis::drawHandPose(frame, d.keypoints, d.keypointScores);
        }
    }, cfg);
