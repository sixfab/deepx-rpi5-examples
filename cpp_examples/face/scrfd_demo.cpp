// ============================================================
// scrfd_demo.cpp  --  SCRFD Face Detection (PPU)
// SCRFD is anchor-free. The PPU returns a DataType::FACE stream
// whose element layout matches DeviceFace_t, but SCRFD reuses
// x/y/w/h as (l, t, r, b) distances-from-centre in grid units,
// and uses layer_idx to select a stride from {8, 16, 32}. Five
// facial landmarks are emitted per face.
// Usage: ./scrfd_demo [--model path] [--source video|camera]
...
class SCRFDDetector {
public:
    explicit SCRFDDetector(const DemoParams& p)
          ...
          m_conf(p.confThreshold), m_iou(p.iouThreshold)
    {
        ...
        if (!m_engine) throw std::runtime_error("Failed to load SCRFD model");
    }
    ...
    std::vector<ppu::FaceDetection> infer(const cv::Mat& bgr) {
        ...
        auto dets = ppu::decodeScrfdPpu(outs, m_inputW, m_inputH,
                                        m_conf, m_iou);
