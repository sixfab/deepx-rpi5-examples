// ============================================================
// yolov8_ppu_demo.cpp  --  Educational PPU demo for YOLOv8
//
// Same inference path as object_detection/yolov8_demo, but with
// extra logging of the first-frame tensor(s) so you can see what
// the Post-Processing Unit actually returns. The PPU computes
// NMS + anchor decode on-chip and hands us a compact DevicePose_t-
// shaped BBOX struct stream:
//
//   tensor[0].type == dxrt::DataType::BBOX
//   tensor[0].shape == {1, N, 1}  where each elem holds:
//     cx (f32), cy (f32), w (f32), h (f32), score (f32), label (u32)
//
// Compared to the "raw logits -> sigmoid -> NMS on CPU" pathway,
// the PPU version removes thousands of CPU ops per frame and is
// the recommended default on DeepX NPUs.
// Usage: ./yolov8_ppu_demo [--model path] [--source video|camera]
...
class YOLOv8PpuDetector {
public:
    explicit YOLOv8PpuDetector(const DemoParams& p)
          ...
          m_conf(p.confThreshold), m_iou(p.iouThreshold)
    {
        ...
        if (!m_engine) throw std::runtime_error("Failed to load YOLOv8 model");
    }
