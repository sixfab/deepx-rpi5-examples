// ============================================================
// ppe_detection_demo.cpp  --  PPE (Hardhat / Vest / Mask) Detection
// YOLOv11 uses the same anchor-free PPU output format as YOLOv8,
// so the decode path is identical to yolov8_ppu_demo.
// Usage: ./ppe_detection_demo [--model path] [--source video|camera]
...
class PPEDetector {
public:
    explicit PPEDetector(const DemoParams& p)
          ...
          m_conf(p.confThreshold), m_iou(p.iouThreshold)
    {
        ...
        if (!m_engine) throw std::runtime_error("Failed to load PPE model");
    }
...
int main(int argc, char** argv) {
    ...
    auto labs = sdk::loadLabels(p.labelPath, labels::PPE_CLASSES);
