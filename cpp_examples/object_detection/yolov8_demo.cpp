// ============================================================
// yolov8_demo.cpp  --  YOLOv8 Object Detection
// Default Ultralytics YOLOv8 — anchor-free, float tensor output
// (no PPU post-processing). DeepX NPU inference driven by the
// shared `deepx_common` library.
// Usage: ./yolov8_demo [--model path] [--source video|camera] [--path file]
//                     [--conf 0.3] [--iou 0.45]
// ============================================================
...
public:
    explicit YOLOv8Detector(const DemoParams& p)
    ...
    std::vector<ppu::Detection> infer(const cv::Mat& bgr) {
        ...
        auto dets = ppu::decodeYolov8Float(outs, m_inputW, m_inputH, m_conf, m_iou);
