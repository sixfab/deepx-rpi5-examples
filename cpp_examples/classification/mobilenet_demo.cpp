// ============================================================
// mobilenet_demo.cpp  --  MobileNetV2 ImageNet Classifier
// Output: [1, 1000] probabilities. Direct resize (no letterbox).
// Same layout as efficientnet_demo — different backbone.
// Usage: ./mobilenet_demo [--model path] [--source video|camera]
//                        [--path file]
// ============================================================
...
public:
    MobileNetClassifier(const DemoParams& p, std::vector<std::string> labels)
        : m_inputH(p.inputHeight), m_inputW(p.inputWidth),
          m_labels(std::move(labels))
    {
        m_engine = sdk::loadEngine(p.modelPath, m_inputH, m_inputW);
        if (!m_engine) throw std::runtime_error("Failed to load MobileNetV2 model");
    }
...
int main(int argc, char** argv) {
    ...
    MobileNetClassifier model(p, labs);
