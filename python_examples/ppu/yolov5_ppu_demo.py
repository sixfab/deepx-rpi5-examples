"""YOLOv5 (PPU) Detection Demo — DeepX SDK
---------------------------------------------
Description: Same network as the yolov5 sync demo, but compiled with the
             DeepX PPU (Post Processing Unit) on. The PPU decodes the
             grid + anchor math on the NPU itself, so instead of three
             FPN tensors we get one flat (N, 32) byte-packed buffer:
             ...
             We still need to apply the YOLOv5 anchor/stride math to lift
             the raw box into model-space pixels, then NMS + unletterbox
             like a normal detector.
Input      : webcam / rpicam / video file / image file
Output     : OpenCV window with bounding boxes and class labels.

Usage:
    python yolov5_ppu_demo.py                                # uses config.py
    python yolov5_ppu_demo.py --source webcam
    python yolov5_ppu_demo.py --model yolov5_ppu.dxnn
"""
...
# YOLOv5 default anchors, one (w, h) row per anchor index (3 per stride).
ANCHORS_BY_STRIDE = {
...
class YOLOv5PPUDetector:
    """Decode the PPU byte-buffer into x1y1x2y2 boxes, scores, class ids."""
    ...
    def postprocess(self, output_tensors):
        # PPU emits one (1, N, 32) byte-packed buffer. If the model wasn't
        # actually compiled with PPU we'll see a different shape and bail.
        ...
        # Reinterpret the same bytes as different dtypes per region.
        ...
        # Standard YOLOv5 head decoding: box centre is offset within grid
        # cell, w/h is squared scaling against the assigned anchor box.
    ...
    def _lookup_anchors(stride: np.ndarray, anchor_idx: np.ndarray):
        """Vectorised per-row anchor (w, h) lookup keyed by stride."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="YOLOv5 PPU Demo — DeepX SDK")
...
def main() -> None:
    ...
    model = YOLOv5PPUDetector(args.model, args.conf, args.iou)
