"""YOLOv10 Demo — DeepX SDK
-------------------------------
Description: NMS-free object detection (COCO-80 by default).
             YOLOv10's head already emits decoded boxes of the form
             [x1, y1, x2, y2, score, class_id], so postprocess is just a
             confidence-threshold filter — no NMS, no class-argmax.
Input      : webcam / rpicam / video file / image file
Output     : OpenCV window with drawn bounding boxes, labels and FPS.

Usage:
    python yolov10_demo.py                                 # uses config.py
    python yolov10_demo.py --source webcam
    python yolov10_demo.py --model yolov10.dxnn
"""
...
class YOLOv10Detector:
    """Decode the already-NMSed YOLOv10 output."""

    def __init__(self, model_path: str, labels: List[str],
                 conf_threshold: float, iou_threshold: float) -> None:
        # iou_threshold is accepted for CLI symmetry but unused — YOLOv10
        # is "end-to-end": the model itself does the duplicate suppression.
        del iou_threshold
    ...
    def postprocess(self, output_tensors) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        # outputs[i] = [x1, y1, x2, y2, score, class_id] in letterboxed space.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="YOLOv10 Demo — DeepX SDK")
...
def main() -> None:
    ...
    model = YOLOv10Detector(args.model, labels, args.conf, args.iou)
