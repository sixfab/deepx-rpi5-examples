"""YOLOv11 Demo — DeepX SDK
-------------------------------
Description: Multi-class object detection (COCO-80 by default).
             Same output schema as YOLOv8/v9 — the postprocess body is
             intentionally identical.
Input      : webcam / rpicam / video file / image file
Output     : OpenCV window with drawn bounding boxes, labels and FPS.

Usage:
    python yolov11_demo.py                                 # uses config.py
    python yolov11_demo.py --source webcam
    python yolov11_demo.py --model yolov11.dxnn
"""
...
class YOLOv11Detector:
    """YOLOv11 head: same as YOLOv8 (transposed (N, 4+C))."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="YOLOv11 Demo — DeepX SDK")
...
def main() -> None:
    ...
    model = YOLOv11Detector(args.model, labels, args.conf, args.iou)
