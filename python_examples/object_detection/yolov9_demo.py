"""YOLOv9 Demo — DeepX SDK
-------------------------------
Description: Multi-class object detection (COCO-80 by default).
             YOLOv9 keeps the YOLOv8 output schema (no objectness; the
             per-class score is the confidence), so the postprocess code
             is identical apart from the renamed class.
Input      : webcam / rpicam / video file / image file
Output     : OpenCV window with drawn bounding boxes, labels and FPS.

Usage:
    python yolov9_demo.py                                  # uses config.py
    python yolov9_demo.py --source webcam
    python yolov9_demo.py --source video --path input.mp4
    python yolov9_demo.py --model yolov9.dxnn
"""
...
class YOLOv9Detector:
    """YOLOv9 head: one tensor of shape (1, 4 + num_classes, num_anchors)."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="YOLOv9 Demo — DeepX SDK")
...
def main() -> None:
    ...
    model = YOLOv9Detector(args.model, labels, args.conf, args.iou)
