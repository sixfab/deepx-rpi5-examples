"""YOLOv12 Demo — DeepX SDK
-------------------------------
Description: Multi-class object detection (COCO-80 by default).
             Same output schema as YOLOv8/v9/v11.
Input      : webcam / rpicam / video file / image file
Output     : OpenCV window with drawn bounding boxes, labels and FPS.

Usage:
    python yolov12_demo.py                                 # uses config.py
    python yolov12_demo.py --source webcam
    python yolov12_demo.py --model yolov12.dxnn
"""
...
class YOLOv12Detector:
    """YOLOv12 head: same as YOLOv8 (transposed (N, 4+C))."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="YOLOv12 Demo — DeepX SDK")
...
def main() -> None:
    ...
    model = YOLOv12Detector(args.model, labels, args.conf, args.iou)
